...
export class CheckoutComponent implements OnInit {
  ...
  public method = { type: '', number: undefined, name: undefined, security_code: undefined, validity: undefined, cpf: undefined };
  ...
  private hasMissingFields(): boolean {
    return !this.method.type || !this.method.name || !this.method.security_code || !this.method.validity;
  }
  ...
  public validatePayment(): void {
    if(!this.hasMissingFields()) {
      ...
      this.activateAlert('error', "Preencha todos os campos para poder prosseguir!");
      setTimeout(() => {
  ...
  private changePanel(panel: String): void {
    if(!this.payed) {
      panel === 'payment' ? this.step = 0 : panel === 'confirmation' ? this.step = 1 : this.step = 2;
    }
  }

  public pay(): void {
    if(!this.hasMissingFields() && this.nickname) {
      this.ctrlCheckout.pay(this.mountData())
      .then(res => {
